...
def list_favourites(user):
    ...
    result = []
    favourites = list(Favourite.objects.filter(user_id=user).values_list('node_id', flat=True))
...
def add_favourite(user, node_id):
    ...
    entry, created = Favourite.objects.get_or_create(user_id=user, node_id_id=node_id)
    if not created:
...
def remove_favourite(user, node_id):
    ...
    num_deleted, details = Favourite.objects.filter(user_id=user, node_id_id=node_id).delete()
    if num_deleted == 0:
